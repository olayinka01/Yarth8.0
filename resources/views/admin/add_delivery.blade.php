...
<!-- BREAKCRUMB -->
		...
		<section class="blog">
			<div class="container">
				<div class="row">
                	<div class="col-md-3">
						...
						<aside class="widget widget_categories ">
							<h4 class="widget-title">ADD DELIVERY</h4>
             ...
             <center><label style="color:#FF0000; display:block;" class="bold">{{ HTML::ul($errors->all()) }}</label><label style="color:green; display:block;" class="bold">{{Session::get('adddelivery_msg')}}</label></center><br/>
              
                  <center><div><h1>Add Delivery</h1></div></center>
                  
                                  <div class="">
                                      {{ Form::open(array('url'=>'admin/add_delivery', 'class'=>'form', 'files'=>'true')) }}

                                        <div class="form login ">
                                            
                                            <label>State <sup>*</sup></label>
                                            {{ Form::text('state', Input::old('state'), array('placeholder'=>'State','required'=>'required', 'class'=>'input-text')) }}
                                            
                                            <label>Price <sup>*</sup></label>
                                            {{ Form::text('price', Input::old('price'), array('placeholder'=>'Price','required'=>'required', 'class'=>'input-text')) }}
                                            
                                            <center>                
                                            {{ Form::submit('Add', array('class'=>'btn btn-13 btn-submit text-uppercase')) }}
                                            </center>
                                        </div>
                                        {{ Form::close() }}
                                   </div>
