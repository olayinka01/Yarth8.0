				...
				<div class="row">
                	<div class="col-md-3">
						...
						<aside class="widget widget_categories ">
							<h4 class="widget-title">VIEW ARTISTS</h4>
					...
					<div class="col-md-9">
   							...
   							<div class="" style="font-size:18px; text-align:center; font-weight:bold;"> </div>
          					
                            
                       <div class="row">
                                  <div class=" box col-md-12 panel panel-primary reviews-container">
                                   
                                  <center><h1 style="color:green">Yarth Artists</h1>
                                 ...
                                 <table class="table table-bordered" >
                                      ...
                                      <tr>
                                    <!-- <td >{{$i}}</td> -->
                                        <td >{{$artist_detail->uid}}</td>
                                        <td >{{$artist_detail->firstname}} &nbsp;{{$artist_detail->lastname}}</td>
                                        <td >{{$artist_detail->email}}</td>
                                        <td >{{$artist_detail->phone}}</td>
                                        <!-- <td >{{--$vendor_detail->bank_name--}}</td> -->
                                        <!-- <td >{{--$vendor_detail->act_no--}}</td>  -->
                                       
                        <td >{{$artist_detail->address}} {{$artist_detail->city}}, {{$artist_detail->state }}</td>
                                       
                                        <td >{{$artist_detail->active}}</td>
                                       
                                       <?php 
                                       $view_artslink = "";
                                       $orderslink = "admin/view_orders/".$artist_detail->uid;
                                       $delete_artistlink = "admin/delete_artist/".$artist_detail->uid;
                                       $activate_artistlink = "admin/activate_artist/".$artist_detail->uid;
                                       $deactivate_artistlink = "admin/deactivate_artist/".$artist_detail->uid;
                                       ?>
                                      <!-- <td ><a href={{URL::to($view_artslink)}} class="btn btn-success">View Products</a></td>-->
                                        ...
                                        <td ><a onclick="activate_artist(this)" href={{URL::to($activate_artistlink)}} class="btn  btn-9">Activate</a>&nbsp;<a onclick="deactivate_artist(this)" href={{URL::to($deactivate_artistlink)}} class="btn btn-8">De-activate</a>&nbsp;<a onclick="delete_art(this)" href={{URL::to($delete_artistlink)}} class="btn btn-8">Delete</a></td> 
