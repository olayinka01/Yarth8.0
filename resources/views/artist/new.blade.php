				...
				<div class="row">
                	<div class="col-md-3 ">
						...
						<aside class="widget widget_categories ">
                            ...
                            <ul>
								<li><a href="{{URL::to('artist.new')}}">Upload Artworks</a></li>
					...
					<div class="col-md-9">
                       ...
                       <!-- FORM UPLOAD -->
						...
						{{ Form::open(array('url'=>'artist.new', 'class'=>'form', 'files'=>'true')) }}
                           ...
                           <p id="loader" style="display:none;" >please wait...</p>
							...
							{{ Form::select('artwork_subcategory', array(), 'None', array('required'=>'required', 'id'=>'artsubcategory', 'class'=>'form-control')) }}
                            </section>
                            
                            <p id="loader" style="display:none;" >please wait...</p>
                            
                             <section class="form-group" id="art_subsubcategory">
							<label id="bold">ARTWORK SUB-SUBCATEGORIES<sup>*</sup></label>
							{{ Form::select('artwork_subsubcategory', array(), 'None', array('required'=>'required', 'id'=>'artsubsubcategory', 'class'=>'form-control')) }}
                            </section>
                            
                            <p id="loader" style="display:none;" >please wait...</p>
                            
                             <section class="form-group" id="art_subsubsubcategory">
							<label id="bold">ARTWORK SUB-SUBSUBCATEGORIES<sup>*</sup></label>
							{{ Form::select('artwork_subsubsubcategory', array(), 'None', array('required'=>'required', 'class'=>'form-control')) }}
                            </section>
                       ...
                       <div class="col-md-8 col-md-offset-2">
                        ...
                        <p> <label id="bold">ARTWORK IMAGE (Dimension = 300px &times; 250px) </label> </p>
                      <div class="pic-box" style="margin:0 auto;">
                   <img id="image-file1" src="img/icons/default.png"/>
                           </div>        
                        ...
                        {{ Form::hidden('artwork_image1', 'secret', array('id' => 'artwork_image1')) }}
							...
							{{ Form::textarea('art_description', '', array('maxlength'=>'350','placeholder'=>'Enter Art Description...', 'class'=>'form-control')) }}
                            </section>
...
<!-- FOOTER -->
 ...
 var cloud = {
	 ...
	 get:function(scid){
		 $("#loader").css({display:"block"});
		 $("#art_subsubcategory").css("display","none").find("select").empty();
		  $.ajax({
			  type:"GET",
			  url:"subsubcategory",
			  data:{
				...
				success: function(res){
					...
					$("#art_subsubcategory").css("display","block").find("select").append(contents);
					},
					error:function(res){
					$("#loader").css({display:"none"});
					console.log(res);
	 ...
	 get:function(sscid){
		 $("#loader").css({display:"block"});
		 $("#art_subsubsubcategory").css("display","none").find("select").empty();
		  $.ajax({
			  type:"GET",
			  ...
			  data:{
				...
				success: function(res){
					...
					$("#art_subsubsubcategory").css("display","block").find("select").append(contents);
					},
					error:function(res){
					$("#loader").css({display:"none"});
					console.log(res);
