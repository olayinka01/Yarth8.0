...
@section('title')
Yarth: Sculptures & Carvings/Ethnic-Cultural-Tribal/Americana
@yield('title')
@stop
                            ...
                            <li>
                                ...
                                <ul class="nav-cat">
                                     ...
                                     @foreach($menusubsubcate20 as $menusubsubcate)
                                            <li>
                                                <a class="current-menu"  href="{{URL::to('buy/artworks/sculptures-carvings/ethnic-cultural-tribal/americana')}}">{{$menusubsubcate->subsubcat_name}}</a>      
                                           </li>
                                           @endforeach
								...
								$newlink="selected-art/".$subsubsubcat_id."/".$artid;
								 ?>
       							...
       							<div class="col-md-6 col-sm-6" id="margintop30" >
									...
									<span style="display:none">{{$art->aid}}</span>  

				<strong> ₦ <span>{{number_format($art->unit_price)}}</span></strong></span>
                
                					<span style="display:none">{{$art->user_id}}</span>
                                    
                                    @if(isset($artist_info))
                                    Art by:<span>{{$artist_info->lastname}} {{$artist_info->firstname}}</span>
                                    @endif
                                    </div>
                       ...
                       {{ Form::open(array('url' => 'home_cart','class'=>'form-inline  nopadding clearfix')) }}
                           ...
                           {{ Form::submit('Add to Cart', array('class' => 'btn btn-9')) }}
                            
                            {{ Form::close() }}
                            			</div>
                                        
                                        <div class="col-md-6">
                       {{ Form::open(array('url' => 'buy_now','class'=>'form-inline  nopadding clearfix')) }}
